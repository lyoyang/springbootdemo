...
public class CalculatorHandler {
    ...
    private Mono<ServerResponse> calculate(final ServerRequest request, final BiFunction<Integer, Integer, Integer> biFunction) {
        final Tuple2<Integer, Integer> operands = extractOperands(request);
        return ServerResponse.ok().body(Mono.just(biFunction.apply(operands.getT1(), operands.getT2())), Integer.class);
    }
